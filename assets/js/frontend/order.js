define(['jquery', 'bootstrap', 'frontend', 'table', 'form'], function ($, undefined, Frontend, Table, Form) {
    ...
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: '/user/order/index' + location.search,
                    add_url: '/user/order/add',
                    edit_url: '/user/order/edit',
                    del_url: '/user/order/del',
                    multi_url: '/user/order/multi',
                    import_url: '/user/order/import',
                    detail_url: '/user/order/detail',
                    refund_url: '/user/order/refund',
                    addTracking_url: '/user/order/addTracking',
                    table: 'order',
                }
            });
            var table = $("#table");
            ...
            table.on('post-common-search.bs.table', function (event, table) {
                ...
                $("input[name='merchant_site_url']", form).addClass("selectpage").data("source", "/user/merchant/site/getSelectItems").data("primaryKey", "url").data("field", "url");
                $("input[name='merchant_group_name']", form).addClass("selectpage").data("source", "/user/merchant/group/getSelectItems").data("primaryKey", "name").data("field", "name");
            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        {checkbox: true},
                        {field: 'risk_flag', title: __('Risk_info'),operate: '=',visible: false, searchList: {"1":__('Yes'),"0":__('No')}},
                        ...
                        {field: 'payment_method_info', title: __('Payment_method'), operate: false, align:'middle',halign:'center',
                            formatter:function (value, row, index){
                                return '<span>'+row.payment_method+'</span>';
                            }},
                        {field: 'merchant_info', title: '商户信息',operate:false,align:'middle',halign:'center',
                            formatter:function (value, row, index){
                                return '<span>分组名称：'+row.merchant_group_name+'</span>'+
                        ...
                        {field: 'paypal_id', title: '交易号', operate: false, align:'middle',halign:'center',
                            formatter:function (value, row, index){
                                let content = '';
                                if(row.transaction_id){
                                    content += '<span>'+row.transaction_id+'</span> <i class="fa fa-copy" style="cursor: pointer"></i>';
                                }
                                return content;
                            }},
                        ...
                        {field: 'duplicate_info', title: __('Duplicate_flag'), operate: false,
                            formatter: function (value,row,index){
                                return row.duplicate_flag===0?'否':'是';
                            }},
            ...
            $(document).on("click", ".btn-export-selected", function () {
                ...
                $.post('/user/order/export',{export_type:'selected',ids:ids},function (res){
                    if(res.code===1){
                        Toastr.info(res.msg)
                    }else{
                        console.log(res);
            ...
            $(document).on("click", ".btn-refund", function () {
                ...
                if(!selectedData.length || !selectedData.length>1){
                    Layer.alert('需选择一条操作数据！');
                    return false;
                }
                var paymentStatus = selectedData[0].payment_status.toLowerCase();
                if(paymentStatus==='completed' || paymentStatus.includes('pending')){
                    var orderStatus = selectedData[0].order_status.toLowerCase();
                    if(orderStatus === 'apply refund'){
                        Layer.alert('退款审核中，请等待！');
                        return false;
                    }
                    Fast.api.open('/user/order/refund/ids/'+selectedData[0]['system_order_id_text'],'订单退款');
                }else{
                    Layer.alert('订单未支付，无法退款！');
                    return false;
                }
            });
            // 指定搜索条件
            $(document).on("click", ".btn-export-all", function () {
                ...
                $.post('/user/order/export',params,function (res){
                    if(res.code===1){
                        Toastr.info(res.msg)
                    }else{
                        console.log(res);
        ...
        aggregate: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'order/aggregate' + location.search,
                    table: 'order',
                }
            });
            var table = $("#table");
                ...
                columns: [
                    [
                        ...
                        {field: 'group_by', title: "分组统计方式", operate: '=',visible: false,
                            searchList:{"merchant_group_name":"商户分组","merchant_site_url":"商户站"}},
